...
def build_model_payload(model_id: str, timestamp: int, config: Dict[str, Any]) -> Dict[str, Any]:
    """构建统一的模型信息载荷"""
    return {
        "id": model_id,
        "object": "model",
...
async def list_models(_: Optional[str] = Depends(auth_manager.verify)) -> Dict[str, Any]:
    ...
    try:
        ...
        model_data: List[Dict[str, Any]] = [
            build_model_payload(model.value, timestamp, Models.get_model_info(model.value))
            for model in Models
        ]
...
async def get_model(model_id: str, _: Optional[str] = Depends(auth_manager.verify)) -> Dict[str, Any]:
    ...
    try:
        ...
        model_info = build_model_payload(model_id, timestamp, config)
