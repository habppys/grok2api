...
class GrokTokenManager:
    ...
    async def _save_data(self) -> None:
        ...
        try:
            async with self._lock:
                data_snapshot = copy.deepcopy(self.token_data)
                
                # 清理 status: active 字段，保持文件整洁
                for token_type in data_snapshot:
                    for token_info in data_snapshot[token_type].values():
                        if token_info.get("status") == "active":
                            token_info.pop("status", None)

            if not self._storage:
                async with self._file_lock:
                    async with aiofiles.open(self.token_file, "w", encoding="utf-8") as f:
                        await f.write(orjson.dumps(data_snapshot, option=orjson.OPT_INDENT_2).decode())
            else:
                await self._storage.save_tokens(data_snapshot)
        except IOError as e:
    ...
    async def record_failure(self, auth_token: str, status: int, msg: str) -> None:
        ...
        try:
            ...
            logger.warning(f"[Token] 失败: {sso[:10]}... (状态:{status}), 原因: {msg}")

            if mark_expired:
                logger.error(f"[Token] 标记失效: {sso[:10]}... (401错误)")
